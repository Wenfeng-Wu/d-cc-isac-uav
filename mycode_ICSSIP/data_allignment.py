...
class EstimateNow(torch.nn.Module):
    def __init__(self, device):
        ...
        self.i1 = iprocess()
        self.linear_out1 = torch.nn.Linear(in_features=16, out_features=8, bias=True)
        ...
        self.i2 = iprocess()
        self.linear_out2 = torch.nn.Linear(in_features=16, out_features=8, bias=True)
        ...
        self.i3 = iprocess()
        self.linear_out3 = torch.nn.Linear(in_features=16, out_features=8, bias=True)
        ...
        # cam4
        self.linear4 = torch.nn.Linear(in_features=2, out_features=8, bias=True)
        self.i4 = iprocess()
        self.linear_out4 = torch.nn.Linear(in_features=16, out_features=8, bias=True)
    ...
    def forward(self, xs):
        ...
        output1 = self.linear_out1(out13)  #映射
        w1 = self.weight1(out11)           #权重
        ...
        output2 = self.linear_out2(out23)  #映射
        w2 = self.weight2(out21)           #权重
        w2 = w2*w_234


        # cam3
        cam_3_xy = xs[6].to(self.device).float()
        cam_3_i = xs[2].to(self.device)
        out31 = self.i3(cam_3_i)  # 处理i
        out32 = self.linear3(cam_3_xy)  # 处理xy
        out33 = torch.cat((out31, out32), dim=1)  # 合并
        output3 = self.linear_out3(out33)  # 映射
        w3 = self.weight3(out31)           #权重
        w3 = w3*w_234

        # cam4
        cam_4_xy = xs[7].to(self.device).float()
        cam_4_i = xs[3].to(self.device)
        out41 = self.i4(cam_4_i)  # 处理i
        out42 = self.linear4(cam_4_xy)  # 处理xy
        out43 = torch.cat((out41, out42), dim=1)  # 合并
        output4 = self.linear_out4(out43)  # 映射
        w4 = self.weight4(out41)           #权重
        w4 = w4*w_234

        output = w1*output1 + w2*output2 + w3*output3 + w4*output4
...
def train(args, model, train_loader, test_loader, device):
    ...
    for epoch in range(1, args.epochs+1):
        ...
        save_dir = os.path.join(os.getcwd(), f"weights/e{epoch}")
...
def test(args, model, test_loader, device):
    ...
    np.save("CamAll_az_diff_list.npy", np.array(az_diff_list))
    np.save("CamAll_el_diff_list.npy", np.array(el_diff_list))
...
if __name__ == "__main__":
    ...
    #model.load_state_dict(torch.load('/home/wfwu/pythonProj/Genesis_uav/mycode/weights_camAll/e100/model.pth'))
    model.load_state_dict(torch.load('C:/FengFeng/wfwuCode/Genesis_uav/mycode/weights_camAll/e100/model.pth'))
